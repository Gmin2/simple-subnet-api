  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const days = /** @type {{ days?: number }} */ (request.query).days || 7; // Default to 7 days
        query += ` AND day >= CURRENT_DATE - INTERVAL '${days} days'`;
        
        if ((/** @type {{ continent?: string }} */ (request.query)).continent) {
          query += ` AND continent = $${paramIndex}`;
          queryParams.push((/** @type {{ continent?: string }} */ (request.query)).continent);
          paramIndex++;
        }
        
        if ((/** @type {{ minerId?: string }} */ (request.query)).minerId) {
          query += ` AND miner_id = $${paramIndex}`;
          queryParams.push((/** @type {{ minerId?: string }} */ (request.query)).minerId);
          paramIndex++;
        }
